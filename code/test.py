...
def makeToyGraph():
    ...
    a = G.add_var_node('a', 3)
    b = G.add_var_node('b', 2)

    Pb = np.array([[0.3], [0.7]])
    G.add_factor_node(Pb, b)

    Pab = np.array([[0.2, 0.8], [0.4, 0.6], [0.1, 0.9]])
    G.add_factor_node(Pab, a, b)
...
def makeTestGraph():
    ...
    c = G.add_var_node('c', 4)
    d = G.add_var_node('d', 5)

    p = np.array([[0.3], [0.7]])
    G.add_factor_node(p, a)

    p = np.array([[0.2, 0.8], [0.4, 0.6], [0.1, 0.9]])
    G.add_factor_node(p, b, a)
